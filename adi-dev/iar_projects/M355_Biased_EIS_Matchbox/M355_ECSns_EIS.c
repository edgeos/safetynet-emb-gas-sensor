...
void UartRxParser()
{
  ...
  else if ((uint8_t)ucInCnt == sizeof(szInSring)-1-PKT_PAYLOAD_LENGTH)
  {
...
{
   build_data_packet(&uart_tx_buffer[0], &uart_tx_length, &meas_result);
...
void ZreZimCal()
{
   float sumZre, sumZim, ph_radians;
   ...
   for(uint32_t i=0;i<testNum;i++)
   {
      ph_radians = ImpResult[i].Phase * PI/180;
